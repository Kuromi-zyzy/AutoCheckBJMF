...
def setup_logger(debug: bool) -> logging.Logger:
    """
    初始化日志记录器。
    始终将签到日志写入 sign_log.txt（UTF-8，追加模式）。
    ...
    # 签到日志文件（始终写入）
    sign_handler = logging.FileHandler("sign_log.txt", encoding="utf-8", mode="a")
    sign_handler.setFormatter(logging.Formatter("%(asctime)s - %(message)s"))
    logger.addHandler(sign_handler)
    # 调试日志文件（仅 debug 模式）
    if debug:
        debug_handler = logging.FileHandler("AutoCheckBJMF.log", encoding="utf-8")
...
def qiandao(
    ...
    for uid, raw_cookie in enumerate(cookies):
        ...
        if not all_matches:
            ...
            logger.info(f"班级[{class_id}] | 暂无可签到任务")

        # ── 对每个签到 ID 发起签到请求 ──
        for match_id in all_matches:
            loc = random.choice(locations)  # 随机选一个位置
